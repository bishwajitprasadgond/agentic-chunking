...
def process_document(doc_files, prompt_text, format_choice):
    ...
    # Global dictionary to store combined responses for all documents
    all_documents = {}
    ...
    for doc in doc_files:
        ...
        all_documents[doc.name] = response  # Store each document's response in a separate dictionary
        combined_responses += f"\nDocument: {doc.name}\nResponse:\n{response}\n"

    # Keep all_documents separate and not merge into document_responses
    # document_responses still only stores individual document responses
    all_documentsdic["All_Documents"] = combined_responses  # Removed this line
